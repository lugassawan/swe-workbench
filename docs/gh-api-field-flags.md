...
- `runtime/reply-and-resolve.sh` (invoked as `bin/swe-workbench-reply-and-resolve`) uses
  `-f body=` for both reply call sites (reply bodies start with `@{author}`); guarded by
