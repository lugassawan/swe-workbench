...
4. If the agent needs one of the six behavioral-contract fragments (`severity-output-contract.md`, `comment-scan.md`, `comment-discipline.md`, `docs-discipline.md`, `external-repo-reading.md`, `lsp.md` — check `shared/agents/` for the current list; code-writing agents need `comment-discipline.md` and `comment-scan.md`, and — unless a `file_set` containment already binds them, as with `swe-workbench:code-impl` — `docs-discipline.md` too, per `check_output_discipline_coverage()`), add a `## Shared references` section at the end of the file holding one sentinel block per fragment used, and write prose pointers to it wherever the agent's body needs to invoke that fragment's rules.
5. Run `python3 scripts/sync-shared-blocks.py --write` then `bash scripts/validate.sh`.
6. Run `pytest tests/test_pi_contract.py -v`. The new agent's frontmatter (`name`, `description`, `model`, `tools`, `skills`) is also read by the Pi Coding Agent's own YAML parser, which is strict where `scripts/validate.py`'s hand-rolled parser is lenient — this is a golden-inventory ratchet, not a schema (see `docs/decisions-ci-validation.md` §1), so it only fails once this repo writes a new frontmatter key or tool token, never on an upstream Pi change. Nothing needs regenerating for the new agent to reach Pi — `pi/extensions/subagent.ts`'s `task` tool reads `agents/<name>.md` directly at dispatch time.
