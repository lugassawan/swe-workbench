...
6. Run `pytest tests/test_pi_contract.py -v`. The new agent's frontmatter (`name`, `description`, `model`, `tools`, `skills`) is also read by the Pi Coding Agent's own YAML parser, which is strict where `scripts/validate.py`'s hand-rolled parser is lenient — this is a golden-inventory ratchet, not a schema (see `docs/plugin-platform-decisions.md` §2), so it only fails once this repo writes a new frontmatter key or tool token, never on an upstream Pi change. Nothing needs regenerating for the new agent to reach Pi — `pi/extensions/subagent.ts`'s `task` tool reads `agents/<name>.md` directly at dispatch time.
