...
| Skill | Triggers | Delegation model |
|---|---|---|
| `ticket-context` | Jira keys (`[A-Z]+-\d+`), `atlassian.net/browse/...`, Confluence wiki URLs, `linear.app/.../issue/...`, `github.com/.../(issues\|pull)/N`, `#N` refs. | Invoked by command bodies as a prelude before subagent delegation. Fetches via `mcp__atlassian__*`, `mcp__linear__*` (aspirational), and `gh` CLI. Returns structured context (title, summary, acceptance criteria, linked refs, recent activity). Does not act on the ticket — only resolves it. A bare key routes to Jira unless a Linear MCP is connected and Atlassian is not (URL-primary + availability tiebreak). |
| `observability-context` | Sentry issue URLs, Sentry event IDs, free-text production-error signals (when a Sentry MCP is connected). | Fetches via `mcp__sentry__*` (aspirational). Returns message/culprit, first-seen, frequency, condensed stack trace, always carrying `**Evidence class:** production signal (Phase-1 framing) — NOT a profile`. Framing input for `workflow-performance-investigation` Phase 1 and `/swe-workbench:debug`'s correctness path only — never satisfies `performance-tuner`'s profile-evidence gate. |
| `comms-context` | Slack permalinks (`<workspace>.slack.com/archives/...`), PagerDuty incident URLs/IDs. | Fetches via `mcp__slack__*` and `mcp__pagerduty__*` (both aspirational). Returns participants, timeline, and status for a thread or incident. Invoked by `/swe-workbench:debug`'s correctness path alongside `observability-context`. |
