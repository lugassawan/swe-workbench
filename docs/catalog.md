...
| `workflow-worktree-session` | "in a worktree", "open the X worktree", "cd into worktree", "switch to worktree", "enter worktree", "exit the worktree", "leave worktree". | Routes to `EnterWorktree(path=…)` for existing worktrees, `EnterWorktree(name=…)` for new ones, and `ExitWorktree(action: "keep"\|"remove")` on the way out. Defers to `superpowers:using-git-worktrees` for full new-worktree feature kickoffs (consent, baseline tests). Forbids `Bash(cd …)` as a session-switch mechanism. |
