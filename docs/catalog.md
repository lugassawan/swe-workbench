...
| `workflow-pr-review-followup` | "/swe-workbench:review --check-followup", "re-check PR after fixes", "reviewer follow-up", "check if feedback was addressed". | Reviewer re-check: ephemeral worktree (`--task "pr-followup-$PR"`), reviewer agent, Jaccard ±5-line dedup, digest report. GitHub-read-only — no submit, no comments, no reactions. |
