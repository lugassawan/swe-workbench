...
class FileDiff:
    ...
    # hunk_idx -> set of removed-line texts *in that hunk*. Keyed per-hunk,
    # not file-wide: a rename+modify commit can coincidentally delete the
    # same one-liner (e.g. "// TODO") in one hunk while adding a genuinely
    # new instance of it elsewhere in the file — file-wide matching would
    # wrongly exempt the new one.
    removed_texts: dict[int, set[str]] = field(default_factory=dict)
...
def parse_diff(text: str) -> list[FileDiff]:
    ...
    for line in text.splitlines():
        ...
        if not line:
            current.view.append(ViewLine(" ", new_line, "", hunk_idx))
            new_line += 1
        elif line.startswith("+") and not line.startswith("+++"):
            current.view.append(ViewLine("+", new_line, line[1:], hunk_idx))
            new_line += 1
        elif line.startswith("-") and not line.startswith("---"):
            current.removed_texts.setdefault(hunk_idx, set()).add(line[1:].strip())
            continue
        elif line.startswith("\\"):
            continue
        else:
            content = line[1:] if line.startswith(" ") else line
            current.view.append(ViewLine(" ", new_line, content, hunk_idx))
            new_line += 1
...
# Covers license-header-plus-generated-notice preambles (e.g. an Apache
# header followed by "Code generated by ... DO NOT EDIT."), not just a bare
# one-line marker. A marker past this window is an accepted miss — see
# test_generated_marker_past_scan_window_is_a_known_miss for the boundary.
_GENERATED_MARKER_SCAN_LINES = 20
...
def _rename_scan_view(fd: FileDiff) -> list[ViewLine]:
    ...
    if not fd.is_rename or not fd.removed_texts:
        return fd.view
    return [
        ViewLine(" ", vl.lineno, vl.text, vl.hunk_idx)
        if vl.kind == "+" and vl.text.strip() in fd.removed_texts.get(vl.hunk_idx, ())
        else vl
        for vl in fd.view
    ]
