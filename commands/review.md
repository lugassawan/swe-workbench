...
**When `--mode` is absent or `--mode general`:** invoke `swe-workbench:workflow-pr-review` via the `Skill` tool with `MODE=auto`, passing the resolved PR number. The skill self-detects first-pass vs. followup (#644) by checking whether this reviewer already has a review on the PR and whether it's still open — `--check-followup <N>` below remains available as an explicit override that always forces `MODE=followup`.
