...
4. **Prompt:** "Reply `post` to submit these as inline comments + review decision, or `skip` to leave as-is." On `skip` (or any other reply), stop — no posting; tear down the worktree in the background using the **same task name Step 2 created** (`rimba remove "review-<mode>-<N>" --force`, or the matching git-fallback branch/worktree cleanup) — a `skip` is a clean exit, not an aborted-mid-scan state, so unlike `workflow-pr-review` Step 5's abort case the worktree is NOT preserved for inspection.
5. **On `post`:** normalize the auditor's documented finding rows into `FINDINGS[]` — `severity` and `body` (fold any extra columns, e.g. `test-reviewer`'s `Category`, into `body`) from every row; `path`/`line` from `File:Line` when present. Set `anchor=inline` when a `File:Line` exists AND the line falls on a `+` line (not a context line) in `git -C "$WT" diff "origin/$BASE"...HEAD`; `anchor=pr-level` otherwise — `dependency-auditor` rows have no `File:Line` and always anchor `pr-level`. Derive `DECISION`: `FINDINGS[]` non-empty → `COMMENT`, empty → `APPROVE` (no footer to parse — these auditors don't emit one). Byline: `` _Reviewed by `<auditor>` ([swe-workbench](https://github.com/lugassawan/swe-workbench))._ `` (the specific agent from the mode table, e.g. `security-auditor`). Invoke `swe-workbench:workflow-pr-review-post` with this payload, then tear down the worktree in the background using the same task name Step 2 created (as in the `skip` case above).
