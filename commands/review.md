...
1. **Preflight:** resolve the repository selected by `gh` once with `PR_REPO=$(gh repo view --json nameWithOwner -q .nameWithOwner)`, then derive `SCOPE_SLUG=$(swe-workbench-repo-scope --repo "$PR_REPO")`. Pass `JSON="/tmp/swe-workbench-pr-review/${SCOPE_SLUG}-${PR}-review-${MODE}.json"` to `swe-workbench-preflight-pr` for `owner`/`repo`/`head_sha`/`base`/`author_login`, plus `gh api /user -q .login` for `current_user`. The mode-scoped state file remains distinct from `swe-workbench:workflow-pr-review`'s first-pass/followup files, while using the gh-selected repo rather than the checkout origin keeps fork/default-repo configurations consistent. Allocate this sub-flow's own run-scoped scratch dir from the same source: `RUN_DIR=$(swe-workbench-new-run-dir "review-${MODE}" "$PR" --repo "$PR_REPO")`. `$RUN_DIR` is a mode-0700 directory under `/tmp/swe-workbench-run/` for ad-hoc artifacts and shares the exact repository slug source used by the state file.
...
3. Run the specialist auditor against `git -C "$WT" diff "origin/$BASE"...HEAD`; print severity-organized findings (unchanged from the existing specialist output above). Pass this scratch boundary: inspect the diff directly when possible; write any materialized diff or ad-hoc artifact under `$RUN_DIR`, never `/tmp` root or an implicit harness scratchpad.
...
5. **On `Post`:** normalize the auditor's documented finding rows into `FINDINGS[]` — `severity`, `issue`, `why`, `fix` from the `Severity`, `Issue`, `Why it matters`, `Suggested fix` columns of every row — never a pre-composed `body`; the posting script renders the comment layout. Map an extra `Category` column (e.g. `swe-workbench:test-reviewer`'s) to `category`, not into the text. `swe-workbench:dependency-auditor` rows (`Severity | Manifest | Package@Version | Signal | Recommended action`) map to `issue` = `` `<Package@Version>` in <Manifest> ``, `why` = `Signal`, `fix` = `Recommended action`. For any other auditor whose columns differ, map the closest columns onto `issue`/`why`/`fix` (e.g. a hotspot table: `issue` = the hotspot, `why` = its evidence, `fix` = the recommended pattern) — never fall back to a `body`. `path`/`line` from `File:Line` when present; on a `pr-level` row omit them unless `File:Line` is a single `path:<integer>` (a range, `N/A`, or a blank must be omitted, not passed — a malformed value rejects the whole batch). Set `anchor=inline` when a `File:Line` exists AND the line falls on a `+` line (not a context line) in `git -C "$WT" diff "origin/$BASE"...HEAD`; `anchor=pr-level` otherwise — `swe-workbench:dependency-auditor` rows have no `File:Line` and always anchor `pr-level`. Derive `DECISION`: at least one row with `severity ∈ {Critical, High}` → `COMMENT`; otherwise `APPROVE` (no footer to parse — these auditors don't emit one; this mirrors the general reviewer's own APPROVE-unless-Critical/High convention rather than flipping to `COMMENT` on any finding regardless of severity). Invoke `swe-workbench:workflow-pr-review-post` with:
