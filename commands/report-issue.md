...
4. **Rank.** Order candidate insights by **prevalence** (cluster size) first; break ties by **recency** — an entry's position in `MEMORY.md` order — with a boost for any cluster the conversation also touches per step 2. Keep the top **5–7** insights.
...
6. **Turn 1 — ranked digest.** Print insights numbered 1..N, most-prevalent first; each entry shows its theme label, a one-line rationale, the memory entries cited (`name` + `type`), and its fenced preview body. End with:
   ```
