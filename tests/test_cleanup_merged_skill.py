...
2. cleanup-merged rimba binary path must handle partial success (worktree gone, branch
   undeleted) by falling through to branch deletion (Step 6, after the #535 reorder that
   moved Residual Sweep to Step 5) rather than aborting.
...
def test_cleanup_merged_rimba_path_handles_partial_success():
    ...
    assert has_partial and has_fallthrough, (
        ...
        "and 'fall through to Step 6' — both phrases are required to catch "
...
def test_cleanup_merged_step6_delegates_to_delete_branches_script():
    """Step 6 must delegate branch deletion to delete-branches.sh via eval.

    The inline git branch -D / git push origin --delete commands were extracted
    into delete-branches.sh (issue #449). Branch deletion was renumbered from
    Step 5 to Step 6 by the #535 reorder (Residual Sweep now runs first, as Step 5,
    so a stale address-feedback-<N> worktree holding the PR's real head branch
    checked out is cleared before this step's git branch -D runs). The skill slice
    for Step 6 must reference the script and document both KEY=VALUE outputs; the
    old inline commands must not appear within the Step 6 slice (they may still
    appear in prose tables elsewhere, so the check is slice-scoped).
    """
    body = SKILL.read_text()
    ...
    assert "git branch -D <headRefName>" not in step6_slice, (
        "git branch -D <headRefName> must not appear inline in Step 6 — "
        "it was extracted into delete-branches.sh"
    )
    assert "git push origin --delete <headRefName>" not in step6_slice, (
        "git push origin --delete <headRefName> must not appear inline in Step 6 — "
        "it was extracted into delete-branches.sh"
    )
...
def test_cleanup_merged_step5_precedes_step6_delete_branches():
    """Residual Sweep (Step 5) must run BEFORE Delete Branches (Step 6).

    Found in PR #539 review: an address-feedback-<N> worktree checks out the PR's
    real head branch directly. If a stale one survives an interrupted flow and
    branch deletion ran first, git would refuse the `git branch -D` and `eval`
    would swallow the failure silently, leaving Step 5 (which would free the
    branch by removing the stale worktree) never reached. The sweep must run
    first so this race cannot occur.
    """
    body = SKILL.read_text()
    step5_idx = body.index("### Step 5 — Residual Sweep")
    step6_idx = body.index("### Step 6 — Delete Branches")
    assert step5_idx < step6_idx, (
        "Step 5 (Residual Sweep) must appear before Step 6 (Delete Branches) — "
        "the sweep must clear a stale address-feedback worktree's branch lock "
        "before delete-branches.sh attempts to force-delete the PR's head branch"
    )


def test_cleanup_merged_step5_documents_dirty_worktree_skip():
    """Step 5's prose must document that dirty worktrees are skipped, not force-removed.

    Found in PR #539 review: reap_one_worktree force-removed worktrees/branches
    with no uncommitted-changes check, risking silent data loss on exactly the
