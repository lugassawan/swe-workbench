...
- `rules/*/examples/**/*.md` — companion example files must be ≤120 lines each. See `docs/extending.md` for the full `examples/` convention (multi-fence `// file:` header rule, visibility ordering).
- `rules/principle-*.md` wiring (`check_unwired_principle_rules`) — every principle rule must be referenced by at least one agent (`language-*` rules are exempt — they're wired dynamically via each agent's "Language rule (required)" clause, not enumerated by name).
- No dangling `swe-workbench:principle-*`/`swe-workbench:language-*` references (`check_no_stale_principle_language_skill_refs`) — those 39 are plain `rules/<name>.md` files, never `Skill`-tool identifiers.
...
- `workflows.md` entry: `` - `swe-workbench:<name>` — <one-line description> ``

The slice is determined by the id prefix: `principle-*` → `principles.md`; `language-*` → `languages.md`; `workflow-*` and `ticket-context`/`*-context` → `workflows.md`; any other prefix defaults to `principles.md`. The validator's `check_catalog_completeness()` enforces that each slice exactly matches the on-disk rules/skills in its prefix group (including that a rule-slice entry's arrow target matches its own name), and that every agent file includes at least one slice via `@./shared/principles.md`, `@./shared/languages.md`, or `@./shared/workflows.md`. Code-touching agents that include `@./shared/principles.md` must also include `@./shared/languages.md` so language-specific rules are always in scope. If the new agent never touches source code, add its stem to the `_NON_CODE_AGENTS` set at the top of `scripts/validate.py` to suppress this check. See `docs/extending.md` for the full recipe.
