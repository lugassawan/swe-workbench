...
10. **Doc-discipline backstop.** Flag (Low/hygiene) unsolicited documentation and misplaced artifacts: net-new ad-hoc doc artifacts — `SUMMARY.md`, `NOTES.md`, `IMPLEMENTATION_NOTES.md`, unsolicited ADRs — and ad-hoc mid-task artifacts of any extension (debug dumps, scratch fixtures, intermediate outputs) with no anchor in the PR description or task scope; and edits to existing docs (`README*`, `docs/*`, any `*.md`) with no such anchor. Never flag doc changes or artifacts the PR description, brief, or the producing agent's own contract names (e.g. framework-detection artifacts, a docs-scoped PR, a wiring row that names `README.md`). Suggested fix is revert, or split into a docs-scoped change. This scoping is unconditional and independent of the Review Decision footer gate below.
