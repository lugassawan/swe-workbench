...
10. **Doc-discipline backstop.** Flag (Low/hygiene) unsolicited documentation: net-new ad-hoc doc artifacts — `SUMMARY.md`, `NOTES.md`, `IMPLEMENTATION_NOTES.md`, unsolicited ADRs — with no anchor in the PR description or task scope; and edits to existing docs (`README*`, `docs/*`, any `*.md`) with no such anchor. Never flag doc changes the PR description or brief names, or docs that are the change's declared subject (e.g. a docs-scoped PR, a wiring row that names `README.md`). Suggested fix is revert, or split into a docs-scoped change. This scoping is unconditional and independent of the Review Decision footer gate below.
