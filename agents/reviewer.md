...
9. **Comment-quality backstop.** Flag unnecessary or over-cap comments — WHAT-not-WHY, restates-the-code, commented-out code, or over-explained / decision-essay (per `principle-clean-code`'s Comment discipline caps and categories) — as Low/hygiene findings, scoped to `+` (added or modified) lines only — this in-diff scoping is unconditional and independent of the "when instructed" Review Decision footer gate below. Also flag a **stale comment**: a pre-existing comment (its own text unchanged, so it sits on a context line) whose described code the diff *did* change nearby — the comment is now WHAT-not-WHY-wrong or misleading, not just uncapped. Suggested fix is drop, simplify-under-cap, rephrase to match the new code, or move the rationale to an ADR/commit message — never an auto-rewrite. Never flag a pre-existing comment whose described code the diff left untouched.
