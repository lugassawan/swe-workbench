...
`swe-workbench-comment-scan`, `swe-workbench-lsp`, `swe-workbench-pr-review-submit`,
`swe-workbench-preflight-commit`, and `swe-workbench-result-check` are the five scripts in this
...
same reason. Unlike `comment-scan` (advisory, correctly fails open), `preflight-commit` and
`result-check` fail closed: an error is a hard non-zero exit with nothing on stdout, never a
