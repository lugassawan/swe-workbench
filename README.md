...
`swe-workbench` bundles the reasoning a careful senior engineer does every day: architectural judgement (Clean Architecture, DDD, SOLID), test discipline (TDD, F.I.R.S.T.), pattern fluency (GoF and beyond), and idiomatic expertise in Bash, C#, Dart, Go, Java, Kotlin, Python, Ruby, Rust, SQL, Swift, and TypeScript. Principle and language guidance lives in plain `rules/*.md` files, not skills: language rules auto-hint by file type (a non-blocking hint fires when a matching file is touched); principle rules are catalog+judgment — surfaced via a session-start catalog and an agent's own reasoning about when one applies, not by trigger-matching. Commands and subagents are there when you want explicit help.
