---
name: agent-reviewer
description: Pi-adapted SWE Workbench agent role. Senior code reviewer — audits diffs for correctness, security, design, and missing tests. Invoke when reviewing a PR, a diff, or a completed feature.
---

# agent-reviewer

This is a pi port of the Claude Code agent `reviewer`. Use it when the requested work matches the role below. Claude-specific frontmatter (`model`, `tools`) is intentionally not preserved because pi does not load Claude agent definitions natively. Use pi's available tools and skills instead.
