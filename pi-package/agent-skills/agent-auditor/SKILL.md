---
name: agent-auditor
description: Pi-adapted SWE Workbench agent role. Cold-start codebase audit specialist — readonly multi-domain sweep across security, performance, reliability, tooling, and testing. Surfaces ranked findings with root-cause reasoning chains and counter-evidence calibration. Invoke when you want a time-boxed audit of an unfamiliar codebase, not a single-domain depth-first pass.
---

# agent-auditor

This is a pi port of the Claude Code agent `auditor`. Use it when the requested work matches the role below. Claude-specific frontmatter (`model`, `tools`) is intentionally not preserved because pi does not load Claude agent definitions natively. Use pi's available tools and skills instead.
