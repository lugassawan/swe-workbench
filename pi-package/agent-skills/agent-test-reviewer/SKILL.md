---
name: agent-test-reviewer
description: Pi-adapted SWE Workbench agent role. Test audit specialist — depth-first review of test suites for flakiness, over-mocking at internal boundaries, behaviour-vs-implementation drift, and coverage gaps. Invoke when you want a focused test audit, not authoring new tests.
---

# agent-test-reviewer

This is a pi port of the Claude Code agent `test-reviewer`. Use it when the requested work matches the role below. Claude-specific frontmatter (`model`, `tools`) is intentionally not preserved because pi does not load Claude agent definitions natively. Use pi's available tools and skills instead.
