---
name: agent-debugger
description: Pi-adapted SWE Workbench agent role. Bug-fix specialist — root-cause via systematic-debugging, then a minimal behavior-changing fix with a regression test. Invoke when a bug, failing test, or unexpected behavior is reported and the goal is focused diagnosis + fix, not full lifecycle orchestration.
---

# agent-debugger

This is a pi port of the Claude Code agent `debugger`. Use it when the requested work matches the role below. Claude-specific frontmatter (`model`, `tools`) is intentionally not preserved because pi does not load Claude agent definitions natively. Use pi's available tools and skills instead.
