...
description: Pi-adapted SWE Workbench agent role. Documentation author — generates README sections, ADRs, ARCHITECTURE/OVERVIEW, and non-obvious inline comments from diffs and conversation context, matching the repo's existing tone and conventions. Invoke when documentation is missing, stale, or drifting from code.
---

# agent-tech-writer

This is a pi port of the Claude Code agent `tech-writer`. Use it when the requested work matches the role below. Claude-specific frontmatter (`model`, `tools`) is intentionally not preserved because pi does not load Claude agent definitions natively. Use pi's available tools and skills instead.
