...
> **Applies:** Version control & collaboration hygiene — atomic commits (one logical change), commit-message quality (imperative subject, body explains why), branching strategy (feature-branch vs trunk-based), rebase vs merge trade-offs, squash vs preserve history, PR description quality (what/why/how-to-test).
...
See `swe-workbench:workflow-pr-review` for review orchestration and `rules/principle-clean-code.md` for the small-change discipline that makes atomic commits tractable.
