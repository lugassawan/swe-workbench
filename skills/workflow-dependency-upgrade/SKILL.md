...
description: Use when walking the dependency-upgrade lifecycle — routine sweep, Dependabot/Renovate PR triage, or major-version migration. Structured runbook — triage+batch → bump regen lockfile → build/test → breakage-triage → PR hygiene. Per-ecosystem command matrix; composes principle-security.
---
