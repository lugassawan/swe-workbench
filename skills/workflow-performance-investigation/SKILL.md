...
- Design-time hot-path review (Big-O, allocation choices, N+1 avoidance) → `cat "$CLAUDE_PLUGIN_ROOT/rules/principle-performance.md"` directly.
...
- **`rules/principle-performance.md`** — design-time discipline (Big-O, allocation, N+1, data locality). Run inline or hand off; never skip the discipline layer.
