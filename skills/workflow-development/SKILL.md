...
**Goal:** Isolated workspace with clean baseline. Detect an existing worktree (rimba MCP/CLI, or plain git) and resume it, or create a new one with the correct branch-prefix and monorepo scope, waiting on dependency install before running tests — full detection, creation, and recovery mechanics live in `reference/branch-resolution.md`.
