...
- **Enter worktree:** `EnterWorktree path=<rimba-output-path>` is the session switch — `cd` only affects a single Bash subprocess and does **not** move the session; never use it to "enter."
- **Resume note:** record the worktree path (`Path:` line from `rimba add`) in the plan. On any continued or resumed session, call `EnterWorktree path=<that-path>` before running commands. If you catch yourself cd-prefixing commands, that is the signal to stop and re-anchor with `EnterWorktree path=<worktree-path>`.
