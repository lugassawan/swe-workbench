...
- **Reclaim install time:** on a long install, background the `rimba add` call so the session is free. Enter the worktree once `Path: <abs-path>` appears and implement immediately. When rimba finishes, reconcile TDD via `git stash` → write failing test → run (**RED**) → `git stash pop` → run (**GREEN**). Skip when install is fast or `post_create` hooks rewrite files you'd edit.
- **Phase 1→2:** if you implemented during install, stash your changes before writing the failing test — then pop after RED to reach GREEN.
