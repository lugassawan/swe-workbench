...
- **Exit (cd-fallback sessions):** if the worktree was entered via `cd` (not `EnterWorktree`), skip `ExitWorktree` — instead, attempt `ExitWorktree(action=keep)`; if it reports a no-op, return to main with `_GCD=$(git rev-parse --git-common-dir); [[ "$_GCD" != /* ]] || cd "${_GCD%/.git}"`. See `workflow-cleanup-merged` Step 3a for the full contract.
