...
- **Monorepo scope:** prefix the task with the service/package name when the change is scoped to one module — `rimba add <service>/<task> [--flag]` → `<prefix>/<service>/<task>` (e.g. `rimba add backend-api/auth-redirect --bugfix` → `bugfix/backend-api/auth-redirect`). Omit for repo-wide changes.
