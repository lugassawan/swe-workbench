...
description: Resiliency principles — fault tolerance, partial failure, blast radius, failure domains, bulkheads, resource isolation, graceful degradation, fail-fast vs fail-soft, health checks, liveness vs readiness probes, cascading and gray failures, idempotency keys, safe retry, deduplication, exactly-once effects, rate limiting, quotas, backpressure, token and leaky buckets, jitter, thundering herds, synchronized retries, and 429 responses. Auto-load when isolating dependencies with bulkheads, planning graceful degradation, choosing fail-fast vs fail-soft, configuring health/readiness/liveness probes, evaluating cascading failures, designing fallback paths, making operations safely retryable with idempotency keys or dedup stores, preventing double execution, designing token-bucket rate limiters, handling 429s and backpressure, or adding jitter to prevent thundering-herd retry spikes.
---
<!-- preload-canary: SWB-PRELOAD-PRINCIPLE-RESILIENCY -->
