...
**Hook-interruption detection (stateless, not event-based).** After the sync, the script runs two checks, either of which sets `HOOK_INTERRUPTED=1`:
...
2. **Targeted subtree-wipe probe (#532)** — checks `$HEAD_REF`'s own worktree specifically: if its root directory still exists but ≥90% of its tracked files (by `git status --porcelain` deletion count vs. the `git ls-files` total) are gone, the hook was interrupted *after* deleting most files but *before* removing the root itself. The missing-root scan above cannot see this case — the directory is still there. A ratio threshold (not "any deletion") avoids flagging a worktree that's just legitimately dirty. Both checks are state-based rather than event-based on purpose: an external kill takes down this script too, so it cannot reliably observe its own interruption — but a stale registration or a wiped subtree on the next run always tells the truth.
...
- **`HOOK_INTERRUPTED=1`**: independent of `WORKTREE_GONE` — either of two states, both meaning the timeout guard (or an external kill on a prior run) caught the post-merge hook mid-`rm`. The script is verify-only here: it signals and documents, it never auto-remediates.
