...
| Residual-sweep artifact retained or removal failed | Any of `.data.retained_worktrees`, `.data.retained_state_files`, `.data.retained_artifacts`, or `.data.failed_removals` is non-empty | Not an abort — Step 5 exits 0 with `status: "partial"`. Report every path/reason in Step 7. Inspect dirty worktrees before commit/discard; attribute or safely remove legacy state/artifacts manually; retry failed removals with the matching guarded cleanup command. |
